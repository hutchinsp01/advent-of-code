f = open("day23-test.txt").read().splitlines()
...
def part1(elves):
    ...
    size_real = max([x.real for x in elves_pos]) - min([x.real for x in elves_pos]) + 1
    size_imag = max([x.imag for x in elves_pos]) - min([x.imag for x in elves_pos]) + 1
    size = size_real * size_imag

    return size - num_elves
...
print(part1(elves_pos))
print(part2(elves_pos))
